...
let rec remove_stutter l =
  match l with
  |prev::after::[] -> if prev = after then prev :: []
      else prev :: after :: []
  |prev::after::t -> if prev = after then remove_stutter (prev::t) 
      else prev::remove_stutter (after::t) 
  |[] -> []
  |t::[] -> [];;
...
let rec persistence_helper n = 
  ...
  let x = add n in
  if x <= 9 then 1
  else 1+ persistence_helper x ;;

let additivePersistence n =
  if n < 10 then
    0
  else persistence_helper n;;

let rec root_helper n = 
  if n <= 0 then 0
  else
    let x = n mod 10 + root_helper (n / 10) in
    if x / 10 >= 1 then
      root_helper x
    else x;;
